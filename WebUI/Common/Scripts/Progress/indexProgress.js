...
//    SetProgressTime(null, "2018/11/14 0:00:00", "2018/11/17 0:00:00")
//});
var _index = 0;//进度
...
function SetProgressTime(fun, startTime, endTime) {
    ...
    indexStart2 = startDate.format("MM/dd HH时") ;
    ...
    indexStart3 = startDate.format("MM/dd HH时");
    ...
    var dateNum = getDateDiff(startTime, endTime);
    var str = '';
    for(var i = 0; i < dateNum; i ++){
        ...
        str += '<p>' + d2.format("MM/dd") + '</p>';
    }
    $(".time_slot").html(str);
    $(".time_slot p").css({ "width": "calc(" + 100 / dateNum + "%)" });
    //设置最大值
...
function SetTime1(value) {
    ...
    //indexStart == new Date(startDate).format("MM/dd HH时");
    indexStart2 == startDate.format("MM/dd HH时");
...
function progressTimeControl(img) {
    if ($(img).attr("title") == "暂停") {
        $(img).attr("title", "开始");
        $(img).css("background-image", "url(../../../Common/Scripts/Progress/img/play.png)");
        window.clearInterval(_mProgressTimer);
    }else {
        $(img).attr("title", "暂停");
        $(img).css("background-image", "url(../../../Common/Scripts/Progress/img/pause.png)");
...
function progressTimeStop() {
    ...
    $("#progressTime_control").css("background-image", "url(../../../Common/Scripts/Progress/img/play.png)");
    $("#scroll_Thumb").css("margin-left", "0px");
    $("#scroll_Track").css("width", "0px");
    ScrollBar.value = 0;
    _index = 0;
