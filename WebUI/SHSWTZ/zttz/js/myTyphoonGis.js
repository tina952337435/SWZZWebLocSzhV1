...
drawWarningLineLayer = function () {
    require([
       ...
       var line = new Polyline({
        ...
        AllBJLayerGraphicLayer.add(polyline);
        ...
        var point = new esri.geometry.Point(yellowPoint[0], yellowPoint[1], map.spatialReference);
        var textsymbol = new TextSymbol(textStyle.hour24.text).//动态设置文本值 
            setColor(new dojo.Color([229, 235, 8])).//setColor设置文本颜色 
            setFont(new esri.symbol.Font("18px").//setFont设置文本大小 
            setWeight(esri.symbol.Font.WEIGHT_BOLD)). //setWeight设置文本粗体 
            setOffset(10, 0).
            setAngle(90);//设置偏移方向 
        var graphic = new esri.Graphic(point, textsymbol);
        AllBJLayerGraphicLayer.add(graphic); 


        point = new esri.geometry.Point(bluePoint[0], bluePoint[1], map.spatialReference);
        textsymbol = new TextSymbol(textStyle.hour48.text).//动态设置文本值 
            setColor(new dojo.Color([0, 4, 212])).//setColor设置文本颜色 
            setFont(new esri.symbol.Font("18px").//setFont设置文本大小 
                setWeight(esri.symbol.Font.WEIGHT_BOLD)). //setWeight设置文本粗体 
            setOffset(10, 0).
            setAngle(90);//设置偏移方向 
...
GetTyphoonList = function (data, typhoon_name) {
    ...
    $.each(data, function (n, value) {
        ...
        strHtml+=GetTyphoonLSLJData(value.points, typhoon_name, value.forecast);
...
//展示台风路径
GetTyphoonLSLJData = function (data, typhoon_names, forecast) {
    try {
...
function getPopupHtml(tfbh, i, strTitle, strLngLat, strFSFL, QY, tfLabelList) {
    ...
    //strHtml += '<table><tbody>';
    //strHtml += '<tr data-theme="m" data-role="none">';
    //strHtml += '<td class="typhoonPopupLabel">中心位置：</td>';
    //strHtml += '<td class="typhoonPopupValue" id="typhoon_lnglat' + divId + '">' + strLngLat+'</td>';
    //strHtml += '</tr>';
    //strHtml += '</tbody></table>';
    //strHtml += '</div>';
    //strHtml += '<div class="typhoonPopupContonthr"></div>';
    //strHtml += '<div>';
    ...
    //strHtml += '<table><tbody>';
    //strHtml += '<tr>';
    //strHtml += '<td class="typhoonPopupLabel">中心气压：</td>';
    //strHtml += '<td class="typhoonPopupValue" id="typhoon_pressure' + divId + '">' + QY + '</td>';
    //strHtml += '</tr>';
    //strHtml += '</tbody></table>';
    ...
    //strHtml += '<table><tbody>';
    //strHtml += '<tr>';
    //strHtml += '<td class="typhoonPopupLabel">风速风力：</td>';
    //strHtml += '<td class="typhoonPopupValue" id="typhoon_fsfl' + divId + '">' + strFSFL+'</td>';
    //strHtml += '</tr>';
    //strHtml += '</tbody></table>';
    //strHtml += '</div>';
    //strHtml += '<div class="typhoonPopupContonthr"></div>';
...
/**************预报台风路径begin**************/
GetResultTFLX_YBS = function (param) {
    ...
    $.each(param, function (n, value) {
        ...
        $.each(items, function (m, infoCol) {
            ...
            addMarker_YB_List(mylatlng, n, j, strIcon, yb_ftm, infoCol);
        });
        IsPolyline_YB(items[0].ID, arrlist, yb_color, arrlistIcon, flightPlanCoordinates_YB);
    });
}
function addMarker_YB_List(mylatlng, i, j, strIcon, yb_ftm, value) {
    try {
        ...
        var strFSFL = value.FS + "米/秒," + GetIconResult(value.COLOR);
        var strQY = value.QY + "百帕";
        var strHtml = getPopupHtml(value.ID, i, strTitle, strLngLat, strFSFL, strQY);
        marker2.content = strHtml;
        if (imgSize == 1) {
            marker2.on('click', markerClick);
            marker2.emit('click', { target: marker2 });
