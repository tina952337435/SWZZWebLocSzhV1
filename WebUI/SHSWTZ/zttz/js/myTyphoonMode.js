    ...
    try {
        ...
        $.each(data, function (n, value) {
            ...
            if (imgSize == 1) {
                if (n == j - 1) {
                    //风圈点
                    strIcon = GetIconFQ(value.COLOR); //"./images/typhoon/tf1.gif";
...
function getPopupHtml(tfbh, i, strTitle, strLngLat, strFSFL, QY, tfLabelList) {
    ...
    strHtml += '<table class="zengshuibutton"><tbody>';
    strHtml += '<tr>';
    strHtml += '<td class="typhoonPopupValue" colspan="2" align="center">';
    strHtml += '<div onclick="openFaAnZengshui(undefined,undefined,\''+tfbh+'\')" style="font-size:13px;padding-top:2px;float:right;cursor:pointer;margin-left:5px;border: 1px solid #fff;padding: 1px 8px; border-radius: 5px;color:white;" title="查看台风增水">增水</div>';
    strHtml+='</td>';
    strHtml += '</tr>';
    strHtml += '</tbody></table>';
